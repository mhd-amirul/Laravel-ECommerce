                ...
                <tbody>
                  ...
                  @if ( isset($shoppingCart) )
                      @foreach ($shoppingCart as $item)
                        <tr>
                          <td class="cart-pic @if($loop->iteration == 1) first-row @endif">
                            <img src="{{ url($item->products->image) }}" alt="" width="170" height="170"/>
                          </td>
                          <td class="cart-title @if ($loop->iteration == 1) first-row @endif">
                            <a href="{{ route('product')."?product=".$item->products->id }}" class="text-dark"><h5>{{ $item->products->name }}</h5></a>
                          </td>
                          <td class="p-price @if ($loop->iteration == 1) first-row @endif">
                            {{-- ${{ $item->products->price }}.00 --}}
                            {{ 'Rp.' . number_format($item->products->price,2,',','.') }}
                          </td>
                          <td class="p-price text-dark @if ($loop->iteration == 1) first-row @endif">{{ $item->quantity }}</td>
                          {{-- <td class="qua-col @if ($loop->iteration == 1) first-row @endif">
                            <div class="quantity">
                              <div class="pro-qty">
                          ...
                          <td class="total-price @if ($loop->iteration == 1) first-row @endif">
                            {{-- ${{ $item->products->price * $item->quantity }}.00 --}}
                            {{ 'Rp.' . number_format($item->products->price * $item->quantity,2,',','.') }}
                          </td>
                          <td class="close-td @if ($loop->iteration == 1) first-row @endif">
